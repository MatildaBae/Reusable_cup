...
# 불러오기
gdf = gpd.read_file('selected_data_with_coverage.geojson')

# 커버하는 주변 카페 수의 평균 계산
average_coverage = gdf['coverage_count'].mean()
print(f"Average number of cafes covered: {average_coverage}")
...
# Calculate the number of nearby cafes
nearby_cafe_count = len(gdf_nearby)

# Add annotation for central cafe (latitude, longitude, name, and number of nearby cafes)
ax.text(
    lon1 + 0.001, lat1,  # x and y position of the text
    f"{cafe_name}\nNearby Cafes: {nearby_cafe_count}",
    fontsize=10,
    color='black',
    bbox=dict(facecolor='white', alpha=0.7, edgecolor='black')
)

# Randomly select 5 nearby cafes from the gdf_nearby DataFrame
selected_nearby_cafes = gdf_nearby.sample(5, random_state=42)  # You can set or omit `random_state` for reproducibility

# Add names for the selected nearby cafes, placing text around them in a circular pattern
for _, nearby_cafe in selected_nearby_cafes.iterrows():
    ...
    # Define the central cafe's text position
    central_text_pos_x = lon1 + 0.001
    central_text_pos_y = lat1

    # Check if the label is too close to the central cafe's text (distance threshold)
    distance_to_central_text = np.sqrt((lon2 + offset_x - central_text_pos_x)**2 + (lat2 + offset_y - central_text_pos_y)**2)
    
    # If the label is too close to the central cafe's text, increase the offset
    if distance_to_central_text < 0.003:  # Adjust threshold to avoid overlap
        offset_x += 0.002  # Move the label further horizontally
        offset_y += 0.002  # Move the label further vertically
